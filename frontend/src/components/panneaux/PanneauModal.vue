...
//  on ne mutate jamais la prop directement
const form = ref({
  reference: "",
  ville: "",
  quartier: "",
...
// Pre-remplit le formulaire quand la prop panneau change
watch(
  () => props.panneau,
  (newPanneau) => {
    if (newPanneau) {
      form.value = {
        ...
        // Clone du tableau pour eviter la mutation
...
async function handleSubmit() {
  try {
    if (isEditMode.value) {
      // Envoie uniquement les champs modifiables
      // On exclut faces, faces_count, faces_libres_count
      const payload = {
